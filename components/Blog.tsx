      ...
      <div className="grid md:grid-cols-3 gap-6">
        {posts.map((p) => (
          <Link 
            key={p.slug} 
            href={`/blog/${p.slug}`} 
            className="group block rounded-sm overflow-hidden hover:scale-[1.01] transition-all duration-300"
          >
            <div className="relative w-full h-48 bg-(--rx-steel) overflow-hidden rounded-sm">
              <Image
                src={p.img}
                alt={p.title}
                fill
            ...
            <span className="text-(--rx-brass) text-xs uppercase tracking-widest mt-4 block font-medium">
              {p.category}
            </span>
            <h3 className="text-(--rx-bone) mt-1 text-lg font-medium group-hover:text-(--rx-action) transition-colors duration-300">
              {p.title}
            </h3>
          </Link>
        ))}

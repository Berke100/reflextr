      ...
      <div className="max-w-7xl mx-auto px-6 grid md:grid-cols-2 gap-12 items-center">
        <div>
          ...
          <p className="text-(--rx-bone)/70 text-sm mt-4 max-w-xl">
            Antrenman programını takip et, kulübe giriş yaparken QR kodunu kullan and
        ...
        <div className="relative h-[480px] w-full max-w-[260px] mx-auto">
          {/* Arkadaki kırmızı parlama efektini koruyoruz */}
          <div className="absolute inset-0 bg-gradient-to-tr from-(--rx-action)/25 to-transparent rounded-[2.5rem] blur-2xl" />
          
          {/* Ekranın içi tekrar siyahımsı (bg-(--rx-ink)) yapıldı */}
          <div className="relative border-4 border-white/10 bg-(--rx-ink) rounded-[2.5rem] h-full overflow-hidden shadow-2xl">
            ...
            <div className="h-full w-full flex flex-col items-center justify-center gap-3 px-6 text-center">
              <span className="font-(--font-display) text-2xl text-(--rx-bone) uppercase tracking-wider">
                REFLEX<span className="text-(--rx-ember)">TR</span>
              </span>
              <span className="text-xs text-(--rx-bone)/40 font-(--font-mono)">
                Uygulama Ekran Önizlemesi
              </span>
              <span className="text-[10px] text-(--rx-bone)/20 mt-2">
                Çok Yakında
              </span>
            </div>
          </div>
